...
*/
#define HAL_UART1_PERCFG_BIT        0x02         // USART1 on P0, Alt-1; so set this bit.
#define HAL_UART1_Px_RTS            0x20         // Peripheral I/O Select for RTS.
#define HAL_UART1_Px_CTS            0x10         // Peripheral I/O Select for CTS.
#define HAL_UART1_Px_RX_TX          0x30         // Peripheral I/O Select for Rx/Tx.
#else
...
#if (HAL_UART_ISR == 2)
  PERCFG &= ~HAL_UART1_PERCFG_BIT;    // Set UART0 I/O location to P0.
#else
  PERCFG |= HAL_UART1_PERCFG_BIT;     // Set UART1 I/O location to P1.
#endif
  PxSEL  |= HAL_UART1_Px_RX_TX;       // Enable Tx and Rx on P1.
  ADCCFG &= ~HAL_UART1_Px_RX_TX;      // Make sure ADC doesnt use this.
...
static void HalUARTOpenISR(halUARTCfg_t *config)
{
  ...
  if (config->flowControl)
  {
    ...
    PxSEL |= HAL_UART1_Px_RTS | HAL_UART1_Px_CTS;
  }
  else
  {
    UxUCR = UCR_STOP;
